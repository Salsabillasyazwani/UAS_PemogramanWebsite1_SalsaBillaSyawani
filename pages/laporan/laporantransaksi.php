...
<head>
    ...
    <title>Laporan Transaksi - U-Manage</title>
    ...
    <style>
        ...
        .bg-cash { background: #eff6ff; color: #1e40af; }
        .bg-qris { background: #fef9c3; color: #854d0e; }
            ...
            <?php if (!empty($transaksi)): ?>
                <?php foreach ($transaksi as $row): ?>
                    <tr data-date="<?= date('Y-m-d', strtotime($row['tgl_transaksi'])) ?>">
                        <td><b><?= htmlspecialchars($row['no_invoice']) ?></b></td>
                        ...
                        <td>
                            <?php 
                                $method = strtolower($row['metode_pembayaran']);
                                $badgeClass = ($method == 'cash') ? 'bg-cash' : 'bg-qris';
                            ?>
                            <span class="badge-method <?= $badgeClass ?>"><?= $row['metode_pembayaran'] ?></span>
                        </td>
            ...
            <?php else: ?>
                <tr id="noDataRow">
                    <td colspan="5" style="text-align: center; padding: 30px; color: #94a3b8;">
                        <i class="fas fa-receipt" style="font-size: 24px;"></i><br>Belum ada data transaksi.
                    </td>
                </tr>
            <?php endif; ?>
        </tbody>
...
<script>
    ...
    function openPop(type) {
        ...
        if(type === 'PDF') {
            ...
            msg.innerText = "Pratinjau laporan Transaksi PDF:";
        } else {
            ...
            msg.innerText = "Pratinjau laporan Transaksi Excel:";
        }
    }
    ...
    function exportToPDF() {
        ...
        doc.setFontSize(16);
        doc.text("Laporan Transaksi Penjualan - U-Manage", 14, 15);
        doc.setFontSize(10);
    ...
    function exportToExcel() {
        const workbook = XLSX.utils.table_to_book(document.getElementById("previewTable"), { sheet: "Transaksi" });
