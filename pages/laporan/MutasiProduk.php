...
try {
    $query = "SELECT 
                ...
                m.jumlah,
                m.keterangan,
                m.tanggal as tgl_mutasi,
                p.nama_produk
              FROM mutasi_stok m
              LEFT JOIN produk p ON m.id_produk = p.id_produk
              WHERE m.ID_user = ?
...
<head>
    ...
    <title>Laporan Mutasi Stok - U-Manage</title>
        ...
        <tbody id="tableBody">
            <?php if (!empty($mutasi)): ?>
                <?php foreach ($mutasi as $row): ?>
                    <tr data-date="<?= date('Y-m-d', strtotime($row['tgl_mutasi'])) ?>">
                        <td><?= date('d/m/Y H:i', strtotime($row['tgl_mutasi'])) ?></td>
                        <td><b><?= htmlspecialchars($row['nama_produk'] ?? 'Produk Terhapus') ?></b></td>
                        <td>
                            <?php 
                                $jenis = $row['jenis_mutasi'];
                                $badgeClass = ($jenis == 'Masuk') ? 'bg-masuk' : 'bg-keluar';
                            ?>
                            <span class="badge-stock <?= $badgeClass ?>"><?= $jenis ?></span>
                        </td>
                        <td style="font-weight: bold; color: <?= ($jenis == 'Masuk' ? '#166534' : '#991b1b') ?>;">
                            <?= ($jenis == 'Masuk' ? '+' : '-') ?> <?= $row['jumlah'] ?>
                        </td>
                        <td style="font-size: 12px; color: var(--text-muted);"><?= htmlspecialchars($row['keterangan']) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr id="noDataRow">
                    <td colspan="5" style="text-align: center; padding: 30px; color: #94a3b8;">
                        <i class="fas fa-history" style="font-size: 24px;"></i><br>Belum ada riwayat mutasi.
                    </td>
                </tr>
            <?php endif; ?>
        </tbody>
...
<script>
    ...
    function openPop(type) {
        ...
        previewTable.innerHTML = `<thead>${tableHead}</thead><tbody>${visibleRows || '<tr><td colspan="5" style="text-align:center">Tidak ada data ditemukan</td></tr>'}</tbody>`;
    ...
    function exportToPDF() {
        ...
        doc.text("Laporan Mutasi Stok - U-Manage", 14, 15);
        doc.autoTable({ html: '#previewTable', startY: 25, theme: 'grid', headStyles: { fillColor: [136, 13, 13] }, styles: { fontSize: 8 } });
        doc.save("Laporan_Mutasi.pdf");
    }

    function exportToExcel() {
        const workbook = XLSX.utils.table_to_book(document.getElementById("previewTable"), { sheet: "Mutasi" });
        XLSX.writeFile(workbook, "Laporan_Mutasi.xlsx");
    }
